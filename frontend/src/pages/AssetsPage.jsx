...
function AssetCard({ asset }) {
  ...
  return (
    <motion.article
      ...
      className="group relative border border-[var(--border)] bg-[var(--surface)] p-4 md:p-5 transition-all duration-200 hover:border-[var(--sungold)]/30 hover:shadow-[var(--shadow-sharp-sm)]"
      ...
      <div className="flex flex-col gap-3 relative z-0">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <h2 id={`asset-title-${asset.id}`} className="font-display font-bold text-[var(--white)] text-[15px] leading-tight">
              {asset.title}
            </h2>
            {asset.category && (
        ...
        {isOther && !asset.external_url && !asset.file_path && (
          <p className="font-mono text-[12px] text-[var(--subtle)] italic">
        ...
        <div className="relative z-10 max-w-[1160px] mx-auto px-4 md:px-8">
          ...
          {loading && (!assets || assets.length === 0) ? (
            <div className="space-y-4">
              {[1, 2, 3].map((i) => (
                <div key={i} className="h-32 bg-[var(--surface)] border border-[var(--border)] animate-pulse" />
              ))}
            </div>
          ) : error ? (
            ...
            <ul className="grid gap-4 list-none p-0 m-0" aria-label="Asset list">
              {sorted.map((asset) => (
                <li key={asset.id}>
